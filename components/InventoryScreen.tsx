      ...
      <div className="bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden">
        <div className="p-8 md:p-12 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-400 font-medium text-center md:text-left">Insira o código exato do produto para consultar detalhes.</p>
          </div>
          ...
          <div className="space-y-4">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-12 sm:pl-16 pr-6 py-5 sm:py-6 bg-slate-50 border-2 border-transparent focus:border-blue-500/20 focus:bg-white rounded-[1.5rem] outline-none text-base sm:text-xl font-bold text-slate-800 transition-all shadow-inner placeholder:text-slate-300"
              />
            </div>

            <button
              onClick={handleSearch}
              className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-black transition-all shadow-lg active:scale-[0.98] flex items-center justify-center gap-2"
            >
        ...
        {/* Exibição do Resultado (Somente Informações do Produto ou Erro) */}
        {hasSearched && (
          <div className="px-8 pb-12 animate-in fade-in zoom-in-95 duration-300">
            {filteredProducts.length > 0 ? (
                ...
                {filteredProducts.map((product) => (
                  <div key={product.codigo} className="p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-md hover:bg-blue-700 active:scale-95 whitespace-nowrap"
                    >
              ...
              <div className="p-12 text-center space-y-4 border-2 border-dashed border-rose-100 rounded-[2rem] bg-rose-50/30">
                ...
                <div>
                    ...
                    <p className="text-rose-400 font-medium">O código "{lastSearchedTerm}" não existe na base atual.</p>
                </div>
              </div>
            )}
          </div>
