...
const OrderScreen: React.FC<OrderScreenProps> = ({ user, products, orderList, onAddItem, onRemoveItem, onUpdateItem, onClear }) => {
  ...
  return (
    <div className="relative -m-4 sm:-m-8 min-h-[calc(100vh-160px)] flex flex-col p-4 sm:p-12 overflow-hidden">
      ...
      />
      {/* Overlay & Blur */}
      <div className="absolute inset-0 z-0 bg-slate-900/50 backdrop-blur-[4px]" />

      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 animate-in fade-in slide-in-from-right-8 duration-700">
        
        {/* LEFT: Order Form */}
        <div className="lg:col-span-5 space-y-4 sm:space-y-6">
          <div className="bg-white/95 backdrop-blur-md rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl border border-white/20 overflow-hidden">
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-grow px-4 py-3 sm:px-5 sm:py-4 bg-slate-100/50 border-2 border-slate-100 rounded-xl sm:rounded-2xl focus:border-blue-500 outline-none font-bold text-sm sm:text-base text-slate-800 transition-all shadow-inner"
                    onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                  />
                  <button onClick={handleSearch} className="bg-slate-900 text-white px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl hover:bg-black font-black text-[10px] sm:text-xs uppercase tracking-widest transition-colors shadow-sm whitespace-nowrap">Buscar</button>
                </div>
              </div>

              {foundProduct && (
                <div className="p-4 sm:p-6 bg-blue-50 border-2 border-blue-100 rounded-xl sm:rounded-[1.5rem] animate-in zoom-in-95 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-3 sm:px-5 sm:py-4 bg-slate-100/50 border-2 border-slate-100 rounded-xl sm:rounded-2xl text-center font-black text-base sm:text-lg shadow-inner outline-none focus:border-blue-500 transition-all"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-3 sm:px-5 sm:py-4 bg-slate-100/50 border-2 border-slate-100 rounded-xl sm:rounded-2xl text-center font-black text-base sm:text-lg text-blue-600 shadow-inner outline-none focus:border-blue-500 transition-all"
                  />
                </div>
              </div>
          ...
          <div className="bg-emerald-600/90 backdrop-blur-md rounded-[1.5rem] sm:rounded-[2rem] p-6 sm:p-8 text-white shadow-2xl shadow-emerald-500/20 flex flex-col justify-between h-32 sm:h-40">
        ...
        <div className="lg:col-span-7 flex flex-col gap-4 sm:gap-6">
          <div className="bg-white/95 backdrop-blur-md rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl border border-white/20 overflow-hidden flex flex-col flex-grow min-h-[400px] sm:min-h-[500px]">
          ...
          {orderList.length > 0 && isAdmin && (
             <div className="flex items-start gap-3 sm:gap-4 p-4 sm:p-5 bg-blue-50/80 backdrop-blur-sm rounded-[1.25rem] sm:rounded-3xl border border-blue-100 mt-4">
