...
import { Trash2, Send, Package, DollarSign, ListOrdered, PackagePlus, AlertCircle, Info, Calculator, FileDown, Mail } from 'lucide-react';
...
const OrderScreen: React.FC<OrderScreenProps> = ({ products, orderList, onAddItem, onRemoveItem, onUpdateItem, onClear }) => {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Solicitacao_Rebaixa");
      XLSX.writeFile(wb, `solicitacao_rebaixa_${new Date().toLocaleDateString().replace(/\//g, '-')}.xlsx`);
    } catch (e) {
  ...
  const handleSendEmail = () => {
    ...
    const subject = `[Rebaixa Pro] Nova Solicitação de Preço - ${new Date().toLocaleDateString()}`;
    
    let bodyText = `Olá,\n\nSegue lista de produtos para rebaixa de preço:\n\n`;
    ...
    bodyText += `Favor anexar a planilha gerada pelo sistema Rebaixa Pro antes de enviar.`;
      ...
      <div className="lg:col-span-5 space-y-4 sm:space-y-6">
        <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden">
          ...
          <div className="p-5 sm:p-8 space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="space-y-2">
                <label className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest text-center block">Sugestão R$</label>
                <input
                  type="number"
                  step="0.01"
                  value={price}
        ...
        <div className="bg-emerald-600 rounded-[1.5rem] sm:rounded-[2rem] p-6 sm:p-8 text-white shadow-xl shadow-emerald-500/20 flex flex-col justify-between h-32 sm:h-40">
            <div className="flex justify-between items-start">
                <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.3em] opacity-60">Total Rebaixa</p>
        ...
        {orderList.length > 0 && (
           <div className="flex items-start gap-3 sm:gap-4 p-4 sm:p-5 bg-blue-50/50 rounded-[1.25rem] sm:rounded-3xl border border-blue-100">
             ...
             <p className="text-[9px] sm:text-[10px] font-bold text-blue-700 leading-relaxed uppercase tracking-tight">
               Dica: O email abre com os dados formatados. Lembre-se de baixar e anexar o Excel gerado antes de enviar.
             </p>
           </div>
        )}
      </div>
